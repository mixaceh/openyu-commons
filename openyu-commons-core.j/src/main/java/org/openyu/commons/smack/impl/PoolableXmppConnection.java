...
import com.newegg.sso.util.ClassUtils;
import com.newegg.sso.util.CollectionUtils;
...
public class PoolableXmppConnection implements XMPPConnection {
	...
	public PoolableXmppConnection(XMPPConnection delegate, ObjectPool<XMPPConnection> pool) {
		...
		if (delegate != null) {
			if (delegate instanceof XMPPTCPConnection) {
				socket = ClassUtils.getDeclaredFieldValue(delegate, "socket");
			}
		}
	}
	...
	protected void removeAllListeners(XMPPTCPConnection conn) {
		...
		Map asyncRecvListeners = ClassUtils.getDeclaredFieldValue(conn, "asyncRecvListeners");
		if (CollectionUtils.notEmpty(asyncRecvListeners)) {
			asyncRecvListeners.clear();
		}
		// addConnectionListener(connectionListener)
		Set<ConnectionListener> connectionListeners = ClassUtils.getDeclaredFieldValue(conn, "connectionListeners");
		if (CollectionUtils.notEmpty(connectionListeners)) {
			connectionListeners.clear();
		}

		// addPacketInterceptor(packetInterceptor, packetFilter)
		@SuppressWarnings("rawtypes")
		Map interceptors = ClassUtils.getDeclaredFieldValue(conn, "interceptors");
		...
		@SuppressWarnings("rawtypes")
		Map sendListeners = ClassUtils.getDeclaredFieldValue(conn, "sendListeners");
		if (CollectionUtils.notEmpty(sendListeners)) {
			sendListeners.clear();
		}

		// addSyncStanzaListener(packetListener, packetFilter)
		@SuppressWarnings("rawtypes")
		Map syncRecvListeners = ClassUtils.getDeclaredFieldValue(conn, "syncRecvListeners");
		if (CollectionUtils.notEmpty(syncRecvListeners)) {
			syncRecvListeners.clear();
		}

		// registerIQRequestHandler(final IQRequestHandler iqRequestHandler)
		@SuppressWarnings("rawtypes")
		Map setIqRequestHandler = ClassUtils.getDeclaredFieldValue(conn, "setIqRequestHandler");
		if (CollectionUtils.notEmpty(setIqRequestHandler)) {
			setIqRequestHandler.clear();
		}
		@SuppressWarnings("rawtypes")
		Map getIqRequestHandler = ClassUtils.getDeclaredFieldValue(conn, "getIqRequestHandler");
		if (CollectionUtils.notEmpty(getIqRequestHandler)) {
