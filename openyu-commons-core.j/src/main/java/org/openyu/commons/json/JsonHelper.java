...
public final class JsonHelper extends BaseHelperSupporter {
	...
	// DateValueProcessor());
	// //Locale
	// jsonConfig.registerJsonValueProcessor(Locale.class, new
	// LocaleValueProcessor());
	//
	// //toBean用
	// //Date, 可轉換的日期格式，即Json串中可以出現以下格式的日期與時間
	// //#issue: ""會無法轉換
	// DateMorpher dateMorpher = new DateMorpher(new String[] {
	// DateHelper.DEFAULT_PATTERN,
	...
	private JsonHelper() {
		throw new HelperException(
				new StringBuilder().append(JsonHelper.class.getName()).append(" can not construct").toString());
	...
	//
	// //集合,陣列,字串有"["視為json陣列
	// if (value instanceof Collection || ArrayHelper.isArray(value) ||
	// isJsonArray(value))
	// {
	...
	// protected static <T> T toBean(JSON json, Class<?> clazz, Map<String,
	// Class<?>> classMap)
	// {
	// T result = null;
	// //JSONArray
	...
	// }
	//
	// public static <T> T toObject(String jsonString, JsonConfig jsonConfig,
	// Class<?> clazz)
	// {
	...
	// * @return Map
	// */
	// public static <K, V> Map<K, V> toMap(String jsonArrayString, Class<?>
	// clazz)
	// {
	//
	...
	// for (Object key : jsonObject.keySet())
	// {
	// result.put((K) key, (V) toBean(jsonObject.getJSONObject((String) key),
	// clazz, classMap));
	// }
	// return result;
	// }
	...
	// }
	//
	// public static <T> T read(InputStream inputStream, JsonConfig jsonConfig,
	// Class<?> clazz)
	// {
